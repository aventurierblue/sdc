...
def nonlinear_sdc(fixed_newton_iteration = 6):
    # instantiate problem
    prob = allencahn_fullyimplicit(nvars=(64,64))
    ...
    while k < k_max:
        ...
        while n < n_max:
            ...
            # Newton
            # vk =  sp.linalg.spsolve(dg, g)
            vn -= sp.linalg.gmres(dg, g, x0=np.zeros_like(vn), maxiter=100, atol=1E-12, callback=count, callback_type='legacy')[0]

        uk = vn.copy()
        newton_res_buffer = np.append(newton_res_buffer, res_newton)
        k += 1

    return k, fixed_newton_iteration, count.get_iter(), np.mean(newton_res_buffer)

def nonlinear_sdc_fisher(fixed_newton_iteration = 6):
    # instantiate problem
    ...
    while k < k_max:
        ...
        fn = np.zeros(nvars * coll.num_nodes, dtype='float64')
        n = 0
        n_max = 100 #fixed_newton_iteration
        res_newton = 0
        tol_newton = 1E-11
        while n < n_max:
            for m in range(coll.num_nodes):
            ...
            # assemble dg
            laplace_dif = sp.kron(sp.eye(coll.num_nodes), prob.A[1:-1, 1:-1])
            growth_dif = prob.lambda0**2 * sp.diags(1.0-(prob.nu + 1) *vn**prob.nu, offsets=0)
            dg = -sp.eye(nvars * coll.num_nodes) + dt * sp.kron(QDmat, sp.eye(nvars)).dot(laplace_dif + growth_dif)
...
if __name__ == "__main__":
    ...
    # nonlinear()
    print("starting")
    nonlinear_sdc_fisher()
    #sdc_newton_frame = pd.DataFrame(columns = ["procedure", "total_sdc", "fixed_newton", "total_gmres", "mean_newton_res"])
    #for fixed_newton_iteration in np.arange(start = 1, stop = 21):
    #        print(fixed_newton_iteration)
    #        res = nonlinear_sdc(fixed_newton_iteration)
    #        sdc_newton_frame.loc[len(sdc_newton_frame.index)] = ["sdc_newton", res[0], res[1], res[2], res[3]]
    #print(sdc_newton_frame.to_string())
    #sdc_newton_frame.to_csv("sdc_newton.csv")
    print("ending")
